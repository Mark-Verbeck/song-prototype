...
export const useSongStore = defineStore('songStore', () => {
  ...
  const fetchSongs = async () => {
    ...
    console.log('SongStore: Fetching songs for user ID:', user.value?.id);
  ...
  const subscribeToSongsChanges = () => {
    ...
    songsChannel = supabase
      .channel(`songs_user_${user.value?.id}`) // Create a unique channel for this user's songs
      .on(
        'postgres_changes',
        {
        ...
        (payload) => {
          ...
          if (eventType === 'UPDATE' && newRecord) {
            ...
            if (index !== -1) {
              ...
              if (newRecord.user_id === user.value?.id) {
          ...
          // Handle inserts (new songs uploaded)
            ...
            if (newRecord.user_id === user.value?.id && !songs.value.some(s => s.id === newRecord.id)) {
